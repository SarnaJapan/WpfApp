...
namespace KelpNet.CL.Common
{
    public class ComputeCommandQueue : ComputeObject, IDisposable
    {
        ...
        public unsafe void Read<T>(ComputeBuffer<T> source, bool blocking, long offset, long region, IntPtr destination, ICollection<ComputeEvent> events) where T : unmanaged
        {
            int eventWaitListSize;
            IntPtr[] eventHandles = ComputeTools.ExtractHandles(events, out eventWaitListSize);
            bool eventsWritable = events != null && !events.IsReadOnly;
            IntPtr[] newEventHandle = eventsWritable ? new IntPtr[1] : null;
            CL10.EnqueueReadBuffer(handle, source.handle, blocking, new IntPtr(offset * sizeof(T)), new IntPtr(region * sizeof(T)), destination, eventWaitListSize, eventHandles, newEventHandle);
